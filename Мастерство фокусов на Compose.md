Привет! Меня зовут Костя, я Android-разработчик в онлайн-кинотеатре Premier. Работая над приложением Premier для Android TV, я столкнулся с тем, что система фокусов в Jetpack Compose устроена сложнее, чем кажется на первый взгляд. Особенно это актуально для TV-приложений и устройств без сенсорного управления, где работа с фокусом играет ключевую роль в навигации.

Документации по этой теме в открытом доступе немного, а специфичных сценариев ещё меньше. Поэтому я решил собрать всё, что узнал и применил на практике, в серию статей, которые помогут вам разобраться в механизме фокусов в Compose, научиться эффективно им управлять и избежать распространённых ошибок.
...
- Введение в фокус в Compose
	- Почему фокус важен в современных интерфейсах
...
Фокус — это механизм, который определяет, какой элемент пользовательского интерфейса «активен» и готов принимать ввод (с клавиатуры, пульта, геймпада и т. д.). Грамотно настроенный фокус критически важен для создания удобной и доступной навигации.

#### Почему фокус важен в современных интерфейсах?

- **Навигация и управление:** 
  Фокус позволяет пользователям перемещаться по интерфейсу с помощью клавиатуры, пульта или других устройств. Это особенно важно для платформ вроде **Android TV** или **Wear OS**, где нет полноценного сенсорного ввода.
- **Accessibility:** 
  Фокус помогает людям с ограниченными возможностями взаимодействовать с интерфейсом. Например, скринридеры определяют текущий элемент по фокусному состоянию и озвучивают его содержимое.
- **Визуальная обратная связь:** 
  Элементы в фокусе обычно выделяются (например, рамкой или изменением цвета), что даёт пользователю мгновенную обратную связь о том, с чем он сейчас работает.

#### Основы работы с фокусом в Jetpack Compose

В Jetpack Compose фокус управляется декларативно, в отличие от традиционной системы Android View, где фокус устанавливался автоматически. Это означает, что каждый элемент, который должен быть фокусируемым, должен быть явно помечен соответствующими модификаторами (например, `focusable()` или `focusTarget()`).
...
- **Modifier.focusGroup** – группирует элементы так, чтобы внутри неё можно было переопределять порядок навигации фокуса.
...
В следующих разделах статьи мы разберём каждый из этих инструментов подробнее, рассмотрим их применение и разберём возможные проблемы при работе с фокусом.

#### Концепция уровней в работе фокусов в Compose

Перед тем как глубже разобраться в том, как управлять фокусом в Compose, важно понять концепцию уровней в иерархии Composable функций. 
...
- **Одномерная навигация (TAB-навигация):** Предполагает последовательное перемещение вперёд и назад между элементами интерфейса. По умолчанию, порядок фокусировки соответствует последовательности объявления элементов в коде. Одномерная навигация осуществляется при нажатии клавиши **Tab** на клавиатуре или с помощью прокрутки на устройствах, таких как умные часы. В этом режиме фокусируется каждый элемент на экране.
    
- **Двумерная навигация:** Позволяет перемещаться влево, вправо, вверх или вниз по интерфейсу. Осуществляется с помощью клавиш со стрелками или кнопок на пультах управления (например, на устройствах Android TV). Кроме того, клавиши **Enter** и **Esc** (или кнопки **Ok** и **Back** на пульте) позволяют переходить на уровень ниже или возвращаться на уровень выше в иерархии функций-композиций.

Чтобы лучше понять концепцию уровней в работе фокусов в Jetpack Compose, рассмотрим практические примеры с использованием `Column` и `Row`.
...
```
![[two-dimensional_navigation_horizontal.webm]]

###### Вертикальное расположение

В следующем примере вместо `Row` используются `Column`, но принцип работы фокуса остаётся таким же.

Теперь фокус сначала перемещается **вертикально** внутри колонок, а затем **горизонтально** между колонками.
...
![[two_dimensional_navigation_vertical.webm]]
...
В этом разделе мы разобрали концепцию уровней в работе фокусов в Jetpack Compose и рассмотрели, как элементы могут взаимодействовать друг с другом в одномерной и двумерной навигации. Мы увидели, что система фокусов в Compose работает «из коробки» и позволяет перемещаться по элементам без дополнительной логики.

Однако в реальных приложениях часто требуется более гибкое управление фокусом: программная установка фокуса, изменение порядка навигации или обработка событий при получении/потере фокуса.

В следующих разделах мы рассмотрим, какие инструменты предоставляет Jetpack Compose для полного контроля над системой фокусов, а также узнаем, как можно настраивать поведение фокуса под конкретные сценарии.
## Управление фокусом и настройка поведения
...
- **`focusable()`** — это более высокий уровень абстракции. Он не только делает элемент доступным для фокуса, но и добавляет нужную семантику для accessibility (например, для скринридеров).
...
**Итог:** Если не знаете, какой модификатор выбрать — берите `focusable`. Он покрывает 90% сценариев. А если нужна полная кастомизация или вам не важна доступность — тогда `focusTarget`.
...
##### Пример: Программный запрос фокуса:
```kotlin
...
Также функция `requestFocus` возвращает значения:
...
##### Пример: Сохранение и восстановление фокуса в `LazyRow`:
```kotlin
val focusRequester = remember { FocusRequester() }
...
Функция `captureFocus` позволяет компоненту захватить фокус и отклонять все внешние запросы на его смену. Это полезно в ситуациях, когда нужно зафиксировать фокус на элементе — например, при валидации ввода или выполнении определённого действия.
...
📌 **Пример: Захват и освобождение фокуса в зависимости от длины текста:**
...
При вводе текста длиннее 5 символов компонент автоматически захватывает фокус, меняя рамку на красную. Если длина текста сокращается до 5 символов или меньше, фокус освобождается, а рамка исчезает. 
...
Хотим заблокировать смену фокуса? — `captureFocus`.  
...
В некоторых сценариях важно сохранить фокус на текущем элементе, чтобы при возвращении пользователь продолжил с того же места. Это можно сделать с помощью функций `saveFocusedChild` и `restoreFocusedChild`:
```kotlin
val focusRequester = remember { FocusRequester() }
...
Чтобы создать группу фокуса, примените модификатор `Modifier.focusGroup()` к контейнеру, содержащему фокусируемые элементы. Это гарантирует, что при навигации с помощью клавиатуры, Dpad или программно с использованием `FocusManager.moveFocus()`, фокус будет перемещаться между элементами внутри группы перед переходом к элементам вне её.
...
Эти свойства позволяют точно определить текущее состояние фокуса и соответствующим образом реагировать на его изменения.
##### Пример
...
На Android TV или устройствах с пультом управления фокус особенно важен, так как пользователи перемещаются по интерфейсу с помощью клавиш навигации Dpad. Можно визуально выделить сфокусированный элемент или отображать дополнительную информацию.
...
- Реакция интерфейса на сфокусированный элемент.
- Показ/скрытие всплывающих подсказок при фокусе.
- Сохранение текущего состояния фокуса для возврата к нему при повторном открытии экрана.

## Рекомендации по работе с фокусами

Работа с фокусом особенно важна при разработке приложений для **Android TV** и при обеспечении **доступности (accessibility)** пользовательских интерфейсов. Ошибки в обработке фокуса могут приводить к неожиданному поведению UI, ухудшать пользовательский опыт и даже вызывать падения приложения.

Вот несколько ключевых рекомендаций, которые помогут вам избежать проблем.
...
Jetpack Compose предлагает **гибкий набор инструментов для работы с фокусом**, но их важно правильно комбинировать. Собрал их в таблицы-подсказки к которым можно обращаться чтобы освежить память и выбрать подходящий функционал.
...
Работа с фокусами в Jetpack Compose требует внимательного подхода. Правильный порядок модификаторов, осмысленное использование встроенных инструментов и тестирование на реальных устройствах помогут избежать проблем с навигацией, обеспечив интуитивно понятный интерфейс. Следуя этим рекомендациям, вы сможете создать удобную, стабильную и предсказуемую систему управления фокусом в вашем приложении.
## Заключение
...
Если вам понравился разбор и вы хотите больше материалов по Android-разработке, , подписывайтесь на мой Telegram-канал: https://t.me/mobile_type. Там я делюсь трендами, реальными кейсами, личным опытом и полезными лайфхаками, которые помогают делать приложения лучше.
